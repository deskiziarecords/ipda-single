...
CONFIG = {
    ...
    "pair":               "BTC/USDT", # examples: "EURUSD=X" (yahoo), "BTC/USDT" (bitget), "EURUSD" (metatrader)
    "start_date":         "2018-01-01",
    "end_date":           datetime.today().strftime("%Y-%m-%d"),
    "interval":           "1d", # examples: "1d", "1h"
...
PAIR = CONFIG["pair"]
PAIR_LABEL = PAIR.replace("=X", "").replace("/", "")
...
# 1. DATA ACQUISITION
# ─────────────────────────────────────────────────────────────────────────────
def fetch_data(source: str, pair: str, start: str, end: str, interval: str) -> pd.DataFrame:
    print(f"[1] Fetching {pair} from {source.upper()} ({start} to {end})...")
    ...
    if source == "yahoo":
        ...
        while True:
            ...
            since = bars[-1][0] + 1
            if len(all_bars) > 10000: # safety limit
                break
...
df = fetch_data(CONFIG["data_source"], PAIR, CONFIG["start_date"], CONFIG["end_date"], CONFIG["interval"])
